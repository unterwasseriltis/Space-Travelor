    ...
    <main className="relative min-h-screen w-full overflow-x-hidden overflow-y-auto bg-[#030611]">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full flex-col gap-6 p-4 md:p-8">
        ...
        <section className="mt-auto grid gap-6 xl:grid-cols-[1fr_500px_1fr]">
          ...
          <aside className="flex flex-col justify-end gap-6">
            <CargoPanel items={inventoryItemLabels} title="Inventar" />
          </aside>
        </section>
      </div>
    </main>
...
function ResourcePanel({
  ...
  return (
    <div className="glass-panel hud-outline grid grid-cols-3 gap-6 rounded-2xl px-6 py-4">
      {resources.map((resource) => (
        <div key={resource.label} className="text-center">
          <p className="text-xl font-bold tracking-widest text-accent">{resource.symbol}</p>
          <p className="mt-1 text-2xl font-black text-white">{resource.value}</p>
        </div>
      ))}
    </div>
  );
